    ...
    fields
    {
        ...
        field(3; "Transaction Type"; Option)
        {
            ...
            begin
                "Loan No." := '';
                Amount := 0;
                if "Transaction Type" = "transaction type"::"CRB Fees" then
                    "Global Dimension 1 Code" := 'FOSA';

                if (("Transaction Type" <> "transaction type"::"CRB Fees") and ("Transaction Type" <> "transaction type"::" ")) then begin
                ...
                if Cust.Get("Member No") then begin
                    ...
                    if "Transaction Type" = "transaction type"::"CRB Fees" then
                        "Amount Balance" := Cust."Ordinary Savings";
                end;
                "Account No" := FnGetFosaAccount('ORDINARY');
            end;
        }
        ...
        field(5; Amount; Decimal)
        {
            ...
            begin
                ...
                if (("Transaction Type" = "transaction type"::"Interest Paid") or ("Transaction Type" = "transaction type"::"Ordinary Savings") or ("Transaction Type" = "transaction type"::Loan) or ("Transaction Type" = "transaction type"::"Loan Repayment")) then
                    //Wrong
                    if "Loan No." = '' then
                ...
                if "Transaction Type" = "transaction type"::"Loan Repayment" then begin
                    Loans2.Reset;
                    Loans2.SetRange(Loans2."Loan  No.", "Loan No.");
                    if Loans2.Find('-') then begin
                        Loans2.CalcFields(Loans2."Outstanding Balance");
                        OutBal := Loans2."Outstanding Balance";
                        if Amount > 0 then
                            if Amount > OutBal then
                                Error('Repayment amount is in excess');
                    end;
                end;
            end;
        }
        ...
        field(51516160; "Account No"; Code[100])
        {
            ...
            if ("Account Type" = const(Vendor)) Vendor."No." where("BOSA Account No" = field("Member No"),
