...
Table 51516441 "Transactions"
{
    ...
    fields
    {
        ...
        field(2;"Account No";Code[20])
        {
            ...
            begin
                ...
                if (Account.Balance <> 0) and (Account.Status = Account.Status::New) then
                  begin
                    Account.Status:=Account.Status::Active;
                    Account.Modify;
                    end;
                ...
                if ObjSignatories.FindSet(true) then
                  begin
        ...
        {
            NotBlank = true;
            TableRelation = if ("Account No"=filter(<>'')) "Transaction Types" where ("Account Type"=filter('ORDINARY'|'RENT COLLECTION'))
            ...
            begin
                ...
                if "Bankers Cheque Type"="bankers cheque type"::Company then begin
                 ...
                 //"Available Balance":=Account.Balance-VarAmtHolder+TransactionCharges."Charge Amount")
                  end;
        ...
        {
            TableRelation = "Bank Account"."No." where ("Cheque Clearing Bank"=filter(Yes));
        ...
        field(69221;"ID Number";Code[10])
        {
            ...
            begin
                ...
                begin
                  //ERROR('Member has been blocked by block type %1',Cust.Blocked);

                  Cust.Reset;
                  Cust.SetRange(Cust."No.","ID Number");
                  Cust.SetRange(Blocked,Cust.Blocked::" ");
                  if Cust.Find('-') then
                  begin
                         "Member No":=Cust."No."
        ...
        field(69225;"Tenant No";Code[20])
        {
            DataClassification = ToBeClassified;
            TableRelation = Vendor."No." where ("Vendor Posting Group"=const('TENANT'),
                                                "Rent Account"=field("Account No"));

            trigger OnValidate()
            begin
                if TransactionTypes.Get("Transaction Type") then begin
                  if Customer.Get("Tenant No") then begin
                    "Transaction Description":=TransactionTypes.Description+' '+Customer.Name;
                    "Tenant Name":=Customer.Name;
                  end;
                end;
            end;
        }
        field(69226;"Tenant Transactions";Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(69227;"Tenant Name";Text[50])
        {
            DataClassification = ToBeClassified;
        }
        field(69228;Archive;Boolean)
        {
            DataClassification = ToBeClassified;
        }
    }
    ...
    var
        ...
        ObjSignatories: Record "Products Signatories Details";
