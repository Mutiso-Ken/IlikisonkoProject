...
Table 51516388 "Receipts & Payments"
{
    ...
    fields
    {
        ...
        field(2; "Account No."; Code[30])
        {
            ...
            if ("Account Type" = const("FOSA Loan")) "Member Register"."No." where("Customer Type" = const(Member))
            else
            if ("Account Type" = const(Vendor)) Vendor."No." where("Creditor Type" = filter("Savings Account"))
            else
            if ("Account Type" = const(Micro)) "Member Register"."No." where("Customer Posting Group" = filter('MICRO'));
        ...
        {
            OptionCaption = 'Deposit Contribution,Ordinary Savings,Unallocated Funds';
            OptionMembers = "Deposit Contribution","Ordinary Savings","Unallocated Funds";
        }
        field(50016; "Registration Date"; Date)
        {
            DataClassification = ToBeClassified;
        }
        field(50017; "Print Extra Receipt?"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(50018; Archive; Boolean)
        {
            DataClassification = ToBeClassified;
        }
    }
