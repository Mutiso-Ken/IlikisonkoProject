    ...
    fields
    {
        ...
        field(3; "Loan Product Type"; Code[25])
        {
            ...
            begin
                ...
                //    IF "Client Name"='' THEN "Client Name":=Cust.Name;
                //    MODIFY;
                //    RefDate:=CALCDATE('<+'+GenSetUp."Minimum Loan Eligibilty New"+'>',Cust."Registration Date");
                //    RejoinDateRef:=CALCDATE('<+'+GenSetUp."Min Loan Eligibility Rejoining"+'>',Cust."Registration Date");
                //    IF Cust.rejoined THEN BEGIN
                //      IF RejoinDateRef>TODAY THEN
                //      ERROR('Member must have been in the sacco for a minimum of 6 months to qualify for this loan!');
                //    END ELSE BEGIN
                //      IF RefDate>TODAY THEN
                //      ERROR('Member must have been in the sacco for a minimum of 3 months to qualify for this loan!');
                //    END;
                //  END;//Reg Date not blank
                // END;//Get Cust

                // //RENTAL Loan
                // IF "Loan Product Type"='RENTAL' THEN BEGIN
                //  Vendor.RESET;
                //  Vendor.SETRANGE("BOSA Account No","Client Code");
                //  Vendor.SETRANGE("Vendor Posting Group",'RENT');
                //  IF Vendor.FINDFIRST THEN BEGIN
                //   //Check account has been operation for more than 3 months
                //    IF Vendor."Date Created"<>0D THEN
                //    RefDate:=CALCDATE('<+3M>',Vendor."Date Created");
                //    IF RefDate>TODAY THEN BEGIN
                //      ERROR('Rental account must have been operational for atleast 3 months!');
                //    END;
                ...
                //      ERROR('Account must have been operational for atleast 3 months!');
                //    END;
                //  END ELSE BEGIN
                //    ERROR('Member must have a Rent Collection Savings Account');
                //  END;
                // END;

                //BIASHARA Loan
                if "Loan Product Type" = 'MOTOR' then begin
                    /*LoansG.RESET;
                    LoansG.SETRANGE("Member No","Client Code");
                    LoansG.SETRANGE(Substituted,FALSE);
                    LoansG.SETFILTER("Amont Guaranteed",'>%1',0);
                    IF LoansG.FIND('-') THEN BEGIN
                      ERROR('Member has guaranteed other Loans');
                    END;*/

                    Vendor.Reset;
                    Vendor.SetRange("BOSA Account No", "Client Code");
                    Vendor.SetRange("Vendor Posting Group", 'BIASHARA');
                    if Vendor.FindFirst then begin
                        //Check account has been operation for more than 3 months
                        if Vendor."Date Created" <> 0D then
                            RefDate := CalcDate('<+3M>', Vendor."Date Created");
                        if RefDate > Today then begin
                            Error('Biashara account must have been operational for atleast 3 months!');
                        end;
                        //    //Check for transactions
                        //    VendorLedgerEntry.RESET;
                        //    VendorLedgerEntry.SETRANGE("Vendor No.",Vendor."No.");
                        ...
                        // ERROR('Member must have a Bishara Savings Account');
                    end;
                end;
                ...
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    //Check if Loan is available for Class B
                    Cust.SetRange(Cust."No.", "Client Code");
                    if Cust.Find('-') then begin
                        if Cust."Member class" = Cust."member class"::"Class B" then begin
                            if not LoanType."Allow class B" then
                                Error('Loan is not available for group B members ');
                        end;
                    end;

                    //Check if member is a permanent resident
                    if LoanType."Check Residence" then begin
                        Cust.SetRange(Cust."No.", "Client Code");
                        if Cust.Find('-') then begin
                            Regions.Reset;
                            Regions.SetRange(Code, Cust.Region);
                            Regions.SetRange("Sacco has Branch?", true);
                            if not Regions.FindFirst then begin
                                Error('Member must be a permanent resident within the Sacco area of operation to qualify for this loan');
                            end;
                    ...
                    if (LoanType."Loan Product Expiry Date" = 0D) or (LoanType."Loan Product Expiry Date" > Today) then begin
                        ...
                        if (Source = Source::BOSA) then begin
                            ...
                            if Cust.Find('-') then begin
                                ...
                                sHARES := Cust."Shares Retained";
                                if sHARES < GenSetUp."Retained Shares" then begin
                                    if "Loan Product Type" <> 'STAFF ADVANCE' then begin
                                        if not "Relax Core Capital Condition" then
                                            Error('This member has not attained the minimum core capital to qualify for this loan');
                                    end;
                                end;
                            end;
                        end;
        ...
        field(4; "Client Code"; Code[50])
        {
            ...
            if (Source = const(FOSA)) "Member Register"."No."
            else
            if (Source = const(MICRO)) "Member Register"."No." where("Group Account" = field("Group Account"))
            else
            if (Source = filter(" ")) "Member Register"."No.";
            ...
            begin
                ...
                if Vendor.Find('-') then
                    ...
                    Error('Member does not have a FOSA account');
                ...
                if "Loan Product Type" = 'KARIBU' then begin
                    ...
                    if Cust.Find('-') then begin
                        ...
                        if "Requested Amount" > Cust."Current Shares" then
                            Error('Member can only apply for a maximun of ksh ' + Format(Cust."Current Shares"));
                    end;
                end;

                // IF ("Loan Product Type"='RENTAL') AND ("Requested Amount">500000) THEN BEGIN
                //  Property.RESET;
                //  Property.SETRANGE("Owner No","Client Code");
                //  Property.SETRANGE("Type of Structure",Property."Type of Structure"::Permanent);
                //  IF Property.FIND('-') THEN BEGIN
                //        DocumentAttachment.RESET;
                //        DocumentAttachment.SETRANGE("No.","Loan  No.");
                //        DocumentAttachment.SETRANGE("Document Type",DocumentAttachment."Document Type"::"Fire Insurance");
                //        IF NOT DocumentAttachment.FINDFIRST THEN
                //          ERROR('Property must be insured against fire and peril,please attach document');
                //  END
                //    ELSE
                //    ERROR('Any amount greater tha ksh 500,000 require that the rental be a permanent structure.');
                // END;
                //******************************Karibu Loan***********************************//
                    ...
                    if ObjLoanOffsets.Find('-') then begin
                        ...
                        if Loan.Find('-') then begin
                            ...
                            if Loan."Outstanding Balance" < (Loan."Amount Disbursed" / 2) then begin
                                ...
                                Error('Member must have paid at least 50% of the existing loan');
                            end;
        ...
        field(9; "Approved Amount"; Decimal)
        {
            ...
            begin
                ...
                if "Repayment Method" = "repayment method"::"Reducing Balance" then begin
                    ...
                    TestField(Installments);
                    LPrincipal := ROUND(LoanAmount / RepayPeriod, 1, '=');
                    LInterest := ROUND(LoanAmount * Interest / 12 * (RepayPeriod + 1) / (200 * RepayPeriod), 1, '=');
        ...
        {
            CalcFormula = max("Member Register".Name where("No." = field("Client Code")));
            Editable = false;
            ...
            begin
                ...
                if LoanType.Get("Loan Product Type") then begin
                    if Format(LoanType."Grace Period") <> '' then begin
                        "Repayment Start Date" := CalcDate(LoanType."Grace Period", "Loan Disbursement Date");
                    end;
                end;
                ...
                if "Loan Product Type" = 'RENTAL' then begin
                    ...
                    Vendor.SetRange("Vendor Posting Group", 'RENT');
                    if Vendor.FindFirst then begin
                        RefDate := CalcDate('<+' + Format(Installments) + 'M>', "Repayment Start Date");
                        if RefDate > Vendor."Rent Agreement Expiry Date" then begin
                            Error('Rent collection agreement expires before loan is fully repaid!');
                        end;
                    end;
                end;
            end;
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Unallocated Funds" | "Auctioneer Paid"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
        ...
        field(53183; "Interest Due"; Decimal)
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Loan No" = field("Loan  No."),
                                                                  "Transaction Type" = filter("CRB Fees"),
                                                                  "Posting Date" = field("Date filter"),
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Auctioneer Paid"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Share Capital" | "Interest Paid" | "Additional Shares"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
        ...
        field(69007; "Outstanding Loan"; Decimal)
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Transaction Type" = filter(Loan | "Loan Repayment" | "CRB Fees" | "Interest Paid"),
                                                                  "Currency Code" = field("Currency Filter"),
                                                                  "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
        ...
        {
            TableRelation = Vendor."No." where("Debt Collector" = filter(Yes));
        ...
        field(69216; "Group Account"; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                if "Group Account" = "group account"::"1" then
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Recovery Account"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Recovery Account" | "Loan Insurance Paid"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            FieldClass = FlowField;
        }
        ...
        field(51516208; "Credit Officer"; Code[20])
        {
            Caption = 'User ID';
            NotBlank = true;
            TableRelation = User."User Name" where("Credit Officer" = filter(Yes));
        ...
        field(51516215; "Loan Stage Description"; Text[25])
        {
            Enabled = false;
        }
        ...
        field(51516222; "KRA Pin No."; Code[10])
        {
        }
        ...
        field(51516236; "Insider Lending"; Boolean)
        {
            CalcFormula = lookup("Member Register".Staff where("No." = field("Client Code")));
            FieldClass = FlowField;
        }
        field(51516237; "Insider Board"; Boolean)
        {
            CalcFormula = lookup("Member Register".Board where("No." = field("Client Code")));
            FieldClass = FlowField;
        }
        ...
        field(51516249; "Stop Interest Accrual"; Boolean)
        {
            DataClassification = ToBeClassified;
            Editable = true;
        }
        field(51516250; "Relax Core Capital Condition"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(51516251; "Updated-SASRA"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(51516252; "Exempt From Interest"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(51516253; "Penalty Due Date"; Date)
        {
            CalcFormula = max("Member Ledger Entry"."Posting Date" where("Loan No" = field("Loan  No."),
                                                                          "Transaction Type" = filter("Penalty Charged")));
            FieldClass = FlowField;
        }
        field(51516254; "Interest Due Date"; Date)
        {
            CalcFormula = max("Member Ledger Entry"."Posting Date" where("Loan No" = field("Loan  No."),
                                                                          "Transaction Type" = filter("Interest Due")));
            FieldClass = FlowField;
        }
        field(51516255; "Exempt interest"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(51516256; "Main Sector"; Code[10])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Main Sector".Code;
        }
        field(51516257; "Sub Sector"; Code[10])
        {
            DataClassification = ToBeClassified;
        }
        field(51516258; "Specific Sector"; Code[10])
        {
            DataClassification = ToBeClassified;
        }
        field(51516259; "Oustanding Penalty"; Decimal)
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Loan No" = field("Loan  No."),
        ...
        field(51516260; "Auctioneer Balance"; Decimal)
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Posting Date" = field("Date filter"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Transaction Type" = filter("Auctioneer charged" | "Auctioneer Paid")));
            FieldClass = FlowField;
        }
        field(51516261; CurrDay; Date)
        {
            FieldClass = Normal;
