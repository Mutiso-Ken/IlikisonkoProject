...
Report 51516499 "Transaction Slip Withdrawal"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Transaction Slip Withdrawal.rdlc';
    ...
    dataset
    {
        dataitem(Transactions; Transactions)
        {
            ...
            column(ExciseDutyAmount; ExciseDutyAmount)
            {
            }
            column(TotalCharge; TotalCharge)
            {
            }
            ...
            begin
                ...
                if Account.Get(Transactions."Account No") then begin
                    ...
                    AvailableBalance := Account."Balance (LCY)";
                ...
                if (Type = 'Withdrawal') and "Use Graduated Charges" = true then begin
                    if "Bulk Withdrawal Appl Done" = true then begin
                        ...
                        ObjGraduatedCharge.SetRange(ObjGraduatedCharge."Notice Status", ObjGraduatedCharge."notice status"::Notified);
                ...
                if (Type = 'Withdrawal') and "Use Graduated Charges" = true then begin
                    if "Bulk Withdrawal Appl Done" = false then begin
                        ObjGraduatedCharge.Reset;
                        ObjGraduatedCharge.SetRange(ObjGraduatedCharge."Notice Status", ObjGraduatedCharge."notice status"::"Prior Notice");
                        if ObjGraduatedCharge.Find('-') then begin
                            repeat
                                if (Amount >= ObjGraduatedCharge."Minimum Amount") and (Amount <= ObjGraduatedCharge."Maximum Amount") then begin
                                    if ObjGraduatedCharge."Use Percentage" = true then begin
                ...
                GetCharges(Transactions."Transaction Type", Transactions.Amount);
                ...
                VarChargeAmount := ChargeAmount;
                ExciseDutyAmount := ChargeAmount * 0.2;
                TotalCharge := ExciseDutyAmount + VarChargeAmount;
            end;
    ...
    local procedure GetCharges(TransactionType: Code[100]; Amount: Decimal)
    begin
        ChargeAmount := SFactory.FnGetCashierTransactionBudding(TransactionType, Amount);
    end;
}
