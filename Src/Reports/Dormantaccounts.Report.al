...
Report 51516558 "Dormant accounts"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Dormant accounts.rdlc';

    dataset
    {
        dataitem("Member Register";"Member Register")
        {
            DataItemTableView = where(Status=filter(Dormant));
            ...
            column(Status_MembersRegister;"Member Register"."No.")
            {
                OptionCaption = 'Dormant';
            }
            column(CurrentShares_MembersRegister;"Member Register"."Member Share Class")
            {
            }
            ...
            column(NoOfDays_MembersRegister;"Member Register"."No Of Days")
            {
            }
            ...
            begin
                ...
                Status := "Member Register".Status;
                 cust.Reset;
                 cust.SetRange(Status);
                 if (cust.Status = cust.Status::Dormant) then
                 Report.Run(51516559,true,false,cust);
            end;
        }
    }
