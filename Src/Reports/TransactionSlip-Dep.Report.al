...
Report 51516630 "Transaction Slip - Dep"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Transaction Slip - Dep.rdlc';
    ...
    dataset
    {
        dataitem(Transactions; Transactions)
        {
            ...
            begin
                Bank.Reset;
                Bank.SetRange(Bank.CashierID, Transactions.Cashier);
                ...
                if Posted = false then begin
                    TCharges := FnGetTransactionCharges("Transaction Type", Amount);
                ...
                TransTime := Transactions."Transaction Date";
                VendorNumber := Transactions.No;
            end;

            trigger OnPostDataItem()
            begin
                if Account.Get(VendorNumber) then begin
                    if GenSetUp."Send Membership Reg SMS" = true then begin
                        SFactory.FnSendSMS('CASHIERTRANS', 'You have Deposited KES ' + Format(TransAmount) + ' on ' + Format(TransTime) + ' to your Account at Nafaka SACCO', VendorNumber, Account."Phone No.");
                    end;
                end;
            end;
    ...
    local procedure FnGetTransactionCharges(TransactionType: Text; TransactionAmount: Decimal) TotalCharge: Decimal
    var
    ...
    begin
        ...
        ObjTransCharges.SetFilter(ObjTransCharges."Minimum Amount", '<=%1', TransactionAmount);
        ObjTransCharges.SetFilter(ObjTransCharges."Maximum Amount", '>=%1', TransactionAmount);
        if ObjTransCharges.FindSet then begin
            repeat
                TotalCharges := TotalCharges + ObjTransCharges."Charge Amount";
                if ObjTransCharges."Include Excise Duty" = true then
                    TotalCharges := TotalCharges + ObjTransCharges."Excise Duty (10%)";
