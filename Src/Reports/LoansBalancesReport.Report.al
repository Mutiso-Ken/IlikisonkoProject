...
Report 51516868 "Loans Balances Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Loans Balances Report.rdlc';

    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            CalcFields = "Outstanding Balance", "Oustanding Interest", "Interest Paid", "Principal Paid", "Scheduled Principal to Date", "Schedule Interest to Date";
            DataItemTableView = sorting("Loan  No.") order(ascending) where(Posted = filter(true), "Outstanding Balance" = filter(> 0));
            PrintOnlyIfDetail = false;
            RequestFilterFields = "Loan  No.", "Main Sector", "Loan Product Type", "Date filter", Posted, "Batch No.", "Captured By", "Outstanding Balance", "Date Approved", "Employer Code", "Loan Status";
            ...
            begin
                ...
                InterestDue := SFactory.FnGetInterestDueFiltered("Loans Register", DateFilter);
                "Loans Register".CalcFields("Loans Register"."Outstanding Balance", "Oustanding Interest");
                ...
                if LoanRepaymentSchedule.FindLast() then begin
                    ScheduledLoanBal := LoanRepaymentSchedule."Loan Balance";
                    LoanArrears := "Loans Register"."Outstanding Balance" + "Loans Register"."Oustanding Interest" - ScheduledLoanBal;
