...
Page 51516616 "Loan Recovery Header"
{
                        ...
                        if "Recovery Type" = "recovery type"::"Attach Defaulted Loans to Guarantors" then begin
                            ...
                            if LoanGuarantors.FindSet then begin
                                ...
                                repeat
                                    ...
                                    LoanDetails."Defaulter Loan" := ROUND(FnGetDefaultorLoanAmount("Loan Distributed to Guarantors", LoanGuarantors."Amont Guaranteed", LoanGuarantors."Total Loans Guaranteed", GCount), 0.1, '=');
                                    LoanDetails.Insert;
    ...
    begin
        if RunningBalance > 0 then begin
            ...
            if LoanApp.Find('-') then begin
                ...
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Repayment",
                GenJournalLine."account type"::Member, LoanApp."Client Code", "Loan Disbursement Date", "Mobile Loan" * -1, 'BOSA', EXTERNAL_DOC_NO,
    ...
    begin
        ...
        if ObjLoanGuarantors.FindSet then begin
            repeat
                LineNo := LineNo + 10000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                GenJournalLine."account type"::Member, ObjLoanGuarantors."Guarantor Number", VarPostingDate, ObjLoanGuarantors."Guarantor Amount Apportioned", 'BOSA', EXTERNAL_DOC_NO,
                ...
                GenJournalLine."account type"::Member, VarMemberNo, VarPostingDate, ObjLoanGuarantors."Guarantor Amount Apportioned" * -1, 'BOSA', EXTERNAL_DOC_NO,
    ...
    begin
        LineNo := LineNo + 10000;
        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
        GenJournalLine."account type"::Member, "Member No", "Loan Disbursement Date", "Net Share Capital" * -1, 'BOSA', EXTERNAL_DOC_NO,
        ...
        GenJournalLine."account type"::Member, "Member No", "Loan Disbursement Date", "Net Share Capital", 'BOSA', EXTERNAL_DOC_NO,
        ...
        if LoansRec.Get("Loan to Attach") then begin
            ...
            if LoanDetails.FindSet then begin
                repeat
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Loan Repayment",
                    GenJournalLine."account type"::Member, LoanDetails."Member No", "Loan Disbursement Date", LoanDetails."Amount from Deposits" * -1, Format(LoansRec.Source), EXTERNAL_DOC_NO,
                    'Guarantor ' + LoanDetails."Guarantor Number", "Loan to Attach");
                    //--------------------RECOVER-----------------------------
                    LineNo := LineNo + 10000;
                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Deposit Contribution",
                    GenJournalLine."account type"::Member, LoanDetails."Guarantor Number", "Loan Disbursement Date", LoanDetails."Amount from Deposits", Format(LoansRec.Source), EXTERNAL_DOC_NO,
