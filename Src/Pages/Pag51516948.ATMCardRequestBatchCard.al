...
Page 51516948 "ATM Card Request Batch Card"
{
    ...
    layout
    {
        area(content)
        {
            ...
            {
                SubPageLink = "Maximun Allocation %"=field("Date Requested");
            }
        }
    }
        ...
        area(creation)
        {
            action(OrderCards)
            {
                ...
                begin
                    ...
                    ObjATMApplications.Reset;
                    ObjATMApplications.SetRange(ObjATMApplications."Maximun Allocation %","Date Requested");
                    ObjATMApplications.SetRange(ObjATMApplications."Order ATM Card",true);
                    if ObjATMApplications.FindSet then begin
                    CardTxtFile.Create('C:\ATMCardApplications\ATM Card Applications.txt');
                    CardTxtFile.CreateOutstream(TxtStream);
                    CardTxtFile.Write(ObjATMApplications.Name);
                    CardTxtFile.Write(ObjATMApplications."Date of Birth");
                    CardTxtFile.Write(ObjATMApplications.Beneficiary);
                    CardTxtFile.Write(ObjATMApplications."New Upload(New)");
                    CardTxtFile.Close;
