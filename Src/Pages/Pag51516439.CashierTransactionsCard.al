...
Page 51516439 "Cashier Transactions Card"
{
    DeleteAllowed = false;
    PageType = Card;
        ...
        area(content)
        {
            group(Transactions)
            {
                ...
                group(FCheque)
                {
                    ...
                    {
                        ApplicationArea = Basic;
                        Caption = 'Cheque Source  Bank Name';
                    }
    ...
    var
        ...
        ObjAccountSignatories: Record "Products Signatories Details";
    ...
    begin
        ...
        if Account.Get("Account No") then begin
            Account.CalcFields(Account.Balance, Account."Uncleared Cheques", Account."ATM Transactions", "Outstanding Discounted Amount");
            ...
            if AccountTypes.Find('-') then begin
                ...
                if Account.Status <> Account.Status::New then begin
                    ...
                    TotalUnprocessed := 0;//Account."Uncleared Cheques";
                    ATMBalance := Account."ATM Transactions";

                    //FD
                    if AccountTypes."Fixed Deposit" = false then begin
                        if Account.Balance < MinAccBal then
                            AvailableBalance := (Account.Balance) - (FeeBelowMinBal + MinAccBal + TotalUnprocessed + ATMBalance + Account."EFT Transactions")

                        else
                            AvailableBalance := (Account.Balance) - (FeeBelowMinBal + MinAccBal + TotalUnprocessed + ATMBalance + Account."EFT Transactions");

                    end else begin
                        //AvailableBalance:=(Account.Balance+Account."Outstanding Discounted Amount") - (FeeBelowMinBal+MinAccBal+TotalUnprocessed+ATMBalance+Account."EFT Transactions"+120);
                        AvailableBalance := (Account.Balance) - (FeeBelowMinBal + MinAccBal + TotalUnprocessed + ATMBalance + Account."EFT Transactions");
        ...
        AvailableBalance := Account.Balance - 120;
        //"N.A.H Balance";
    ...
    begin
        ...
        if TellerTill.Find('-') then begin
            ...
            if ChequeTypes.Get("Cheque Type") then begin
                ...
                //CR
                //---------------1. Credit FOSA (with Transaction Amount)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", Amount * -1, DActivity, "Cheque No", "Transaction Type", '');
                //DR
                //---------------1. Debit Clearing BANK (with Transaction Amount)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"Bank Account", "Cheque Clearing Bank Code", "Transaction Date", Amount, DActivity, "Cheque No", 'Cheque Deposit-' + "Cheque No", '');

                //-----------------Cheque Discounting Ledger Entries-----------------------------------------------------------------------
                ObjDiscountingLedger.Init;
                ObjDiscountingLedger.No := No;
                ObjDiscountingLedger."External Transaction No" := No;
                ObjDiscountingLedger."Cheque No" := "Cheque No";
                ObjDiscountingLedger.Amount := -Amount;
                ObjDiscountingLedger.Credit := Amount;
                ObjDiscountingLedger."Transaction Type" := ObjDiscountingLedger."transaction type"::Deposit;
    ...
    begin
        ...
        if ("Transaction Type" = 'WITHDRAWAL') or ("Transaction Type" = 'DEPOSIT') then begin
            //--------------------------A. Cash Deposit Transaction---------------------------------------------------------------------------------------------------------------------------------------------------
            if Type = 'Cash Deposit' then begin
                //---------------1. Credit FOSA (with Transaction Amount)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", Amount * -1, DActivity, "ID No", "Transaction Description", '');
                //GenJournalLine."Account Type"::Vendor,"Account No","Transaction Date",Amount*-1,DActivity,"ID No","Transaction Type"+'-'+"Member No"+'-'+"Member Name",'');
                //---------------2. Debit TELLER (with Transaction Amount)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"Bank Account", TillNo, "Transaction Date", Amount, DActivity, "Account No", "Transaction Description", '');
            end;
            //----------------------------------------------END---------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if Type = 'Withdrawal' then begin
                //DR
                //---------------1. Debit FOSA (with Transaction Amount)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", Amount, DActivity, "ID No", "Transaction Type" + '-' + "Member No" + '-' + "Member Name", '');
                //--------------2. Debit FOSA (With Transaction Charges)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", ChargeAmount, DActivity, "ID No", 'Withdrawal Charges', '');
                //--------------3. Debit FOSA(With Excise Duty)--------------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", ChargeAmount * GenSetup."Excise Duty(%)" / 100, DActivity, "ID No", 'Excise Duty', '');

                //Charge SMS Fee(Debit FOSA Account)
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", GenSetup."SMS Fee Amount", DActivity, "ID No", 'Withdrawal SMS Fee', '');

                //CR
                //---------------1. Credit TELLER (with Transaction Amount)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"Bank Account", TillNo, "Transaction Date", Amount * -1, DActivity, "Account No", CopyStr("Transaction Type", 1, 5) + '-' + "Member No" + ' ' + "Member Name", '');

                //---------------2. Credit Income(With Transaction Charges)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", ChargeGL, "Transaction Date", ChargeAmount * -1, DActivity, "Account No", "Transaction Type" + '-' + "Member Name", '');

                //---------------3. Credit Excise Duty GL---------------------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", GenSetup."Excise Duty Account", "Transaction Date", (ChargeAmount * GenSetup."Excise Duty(%)" / 100) * -1, DActivity, "Account No", "Transaction Type" + '-' + "Member Name", '');

                //Earn SMS Charge(Credit G/L Account)
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", GenSetup."SMS Fee Account", "Transaction Date", -GenSetup."SMS Fee Amount", DActivity, "ID No", 'Withdrawal SMS Fee', '');
            end;
            //----------------------------------------------END---------------------------------------------------------------------------------------------------------------------------------------------------------------

        end else begin

            //--------------------------A. Rent Deposit Transaction---------------------------------------------------------------------------------------------------------------------------------------------------
            if "Transaction Type" = 'RENTDEPOSIT' then begin
                //---------------1. Credit Customer (with Transaction Amount)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Tenant No", "Transaction Date", Amount * -1, DActivity, "ID No", "Transaction Type" + '-' + "Member No" + '-' + "Member Name", '');
                //---------------2. Debit TELLER (with Transaction Amount)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"Bank Account", TillNo, "Transaction Date", Amount, DActivity, "Account No", CopyStr("Transaction Type", 1, 3) + '-' + "Member No" + '-' + "Member Name", '');
            end;
            //----------------------------------------------END---------------------------------------------------------------------------------------------------------------------------------------------------------------

            //--------------------------A. Monthly Rent Transaction---------------------------------------------------------------------------------------------------------------------------------------------------
            if "Transaction Type" = 'RENT' then begin
                //---------------1. Credit Customer (with Transaction Amount)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", Amount * -1, DActivity, "ID No", "Transaction Type" + '-' + "Tenant No" + '-' + "Tenant Name", '');
                //---------------2. Debit TELLER (with Transaction Amount)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"Bank Account", TillNo, "Transaction Date", Amount, DActivity, "Account No", CopyStr("Transaction Type", 1, 3) + '-' + "Tenant No" + '-' + "Tenant Name", '');
                //--------------2. Debit FOSA (With Transaction Charges)------------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::Vendor, "Account No", "Transaction Date", ChargeAmount, DActivity, "ID No", 'Rent Collection Commission', '');

                //---------------2. Credit Income(With Transaction Charges)----------------------------------------------------------------
                LineNo := LineNo + 1000;
                SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
                GenJournalLine."account type"::"G/L Account", ChargeGL, "Transaction Date", ChargeAmount * -1, DActivity, "Account No", 'Rent Collection Commission', '');
            end;
            //----------------------------------------------END---------------------------------------------------------------------------------------------------------------------------------------------------------------



        end;//End of RENT Refund/Deposit
        ...
        if Trans.Find('-') then begin
            if Type = 'Cash Deposit' then begin
                if Account.Get("Account No") then begin
                    if GenSetup."Send Cash Withdrawal SMS" = true then begin
                        SFactory.FnSendSMS('CASHIERTRANS', 'Your Account has been credited with KES ' + Format(Amount) + ' on ' + Format("Transaction Date") + ' at ' + Format("Transaction Time") + ' at Chuna SACCO', "Account No", Account."Phone No.");
                    end;
                end;
            ...
            if Type = 'Withdrawal' then begin
                ...
                if Account.Get("Account No") then begin
                    if GenSetup."Send Cash Withdrawal SMS" = true then begin
                        SFactory.FnSendSMS('CASHIERTRANS', 'You have Withdrawn KES ' + Format(Amount) + ' on ' + Format("Transaction Date") + ' ' + Format("Transaction Time") + ' from your Account at Chuna SACCO', "Account No", Account."Phone No.");
                    end;
                end;
            end;
        end;
    ...
    begin
        // ChargeAmount:=SFactory.FnGetCashierTransactionBudding("Transaction Type",Amount);
        // ChargeGL:='30312';
    end;
}
