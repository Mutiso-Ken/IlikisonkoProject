...
Page 51516385 "Loan Application Card(New)"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Relax Core Capital Condition"; "Relax Core Capital Condition")
                {
                    ApplicationArea = Basic;
                }
                ...
                field("Insider Lending"; "Insider Lending")
                {
                    ApplicationArea = Basic;
                    Caption = 'Board/Staff Loan';
                }
                ...
                field("Penalty Amount"; "Penalty Amount")
                {
                    ApplicationArea = Basic;
                    Visible = false;
                }
                field("Lumpsum Amount Charge"; "Lumpsum Amount Charge")
                {
                    ApplicationArea = Basic;
                    Visible = false;
                }
                ...
                field("Loan Appraisal Fee"; "Loan Appraisal Fee")
                {
                    ApplicationArea = Basic;
                    Editable = false;
                }
                field("Loan Processing Fee"; "Loan Processing Fee")
                {
                    ApplicationArea = Basic;
                    Editable = false;
                }
            ...
            group(Loan)
            {
                ...
                action("Loan Appraisal")
                {
                    ...
                    begin
                        if "Approved Amount" = 0 then
                            Error('Approved amount cannot be 0');
                        LoanApp.Reset;
                        LoanApp.SetRange(LoanApp."Loan  No.", "Loan  No.");
                        if LoanApp.Find('-') then begin
                ...
                action("Move to Appraisal")
                {
                    ...
                    begin
                        ...
                        if "Loan Product Type" = 'SCHFEES' then begin
                            DocumentAttachment.Reset;
                            DocumentAttachment.SetRange("No.", "Loan  No.");
                            DocumentAttachment.SetRange("Document Type", DocumentAttachment."document type"::"Fee Structure");
                            if not DocumentAttachment.FindFirst then
                                Error('Please attach the Fee Structure');
                        end;

                        //  IF "Loan Product Type"='RENTAL' THEN BEGIN
                        //   DocumentAttachment.RESET;
                        //   DocumentAttachment.SETRANGE("No.","Loan  No.");
                        //   DocumentAttachment.SETRANGE("Document Type",DocumentAttachment."Document Type"::"Title Deed");
                        //   IF NOT DocumentAttachment.FINDFIRST THEN
                        //     ERROR('Please attach the Title/Allotment letter');
                        //  END;

                        /*IF "Loan Product Type"='BORESHA' THEN BEGIN
                         DocumentAttachment.RESET;
                         DocumentAttachment.SETRANGE("No.","Loan  No.");
                         DocumentAttachment.SETRANGE("Document Type",DocumentAttachment."Document Type"::Invoice);
                         IF NOT DocumentAttachment.FINDFIRST THEN
                           ERROR('Please attach Invoice from supplier');

                         DocumentAttachment.RESET;
                         DocumentAttachment.SETRANGE("No.","Loan  No.");
                         DocumentAttachment.SETRANGE("Document Type",DocumentAttachment."Document Type"::"Payment Voucher");
                         IF NOT DocumentAttachment.FINDFIRST THEN
                           ERROR('Please attach the payment Voucher');
                        END;
                        */
                        if "Loan Product Type" = 'LIVESTOCK' then begin
                            DocumentAttachment.Reset;
                            DocumentAttachment.SetRange("No.", "Loan  No.");
                            DocumentAttachment.SetRange("Document Type", DocumentAttachment."document type"::"Livestock Holding");
                            if not DocumentAttachment.FindFirst then
                                Error('Please attach prove of ownership of permanent and secure holding ground for livestock');
                        end;
                ...
                action("Notify Guarantors")
                {
                    ...
                    begin
                        ...
                        /* LoanGuar.RESET;
                         LoanGuar.SETRANGE(LoanGuar."Loan No","Loan  No.");
                        // MESSAGE(FORMAT("Loan  No."));
                         IF LoanGuar.FIND('-') THEN
                           BEGIN
                             REPEAT
                               Cust.RESET;
                               Cust.SETRANGE(Cust."No.",LoanGuar."Member No");
                               //MESSAGE(FORMAT(LoanGuar."Member No"));
                               IF Cust.FIND('-') THEN
                                 BEGIN
                                   //FnSendGuarantorAppSMS("Loan  No.");
                       //            SFactory.FnSendSMS('LOAN GUARANTORS','You have guaranteed Mr./Mrs./M/s ' +"Client Name"+' with Kes.'+FORMAT(LGuarantors."Amont Guaranteed")+' for a loan application of Kes. '+FORMAT("Requested Amount")
                       //            +'. If not, let us know. Thanks.',Cust."FOSA Account No.",Cust."Phone No.");
                                 END;
                         ...
                         "Notify Guarantor SMS":=TRUE;
                         MODIFY;*/
                        SFactory.FnSendSMS('LOANAPP',
                        'Dear Mr./Mrs./M/s ' + "Client Name" + ' your loan application form has been received. Please wait as it goes through the appraisal system. You will be notified of the outcome. Thanks. ',
    ...
    begin
        ...
        SMSMessage."SMS Message":='Dear Mr./Mrs./M/s ' +"Client Name"+
        ', your loan application form has been received. Please wait as it goes through the appraisal system. You will be notified of the outcome. Thanks. '
    ...
    begin
        ...
        if LGuarantors.Find('-') then begin
            //AmoutGuarantee:=LGuarantors."Amont Guaranteed";
            //MESSAGE(FORMAT(AmoutGuarantee));
            repeat
                ...
                SMSMessage."Entered By" := UserId;
                SMSMessage."Sent To Server" := SMSMessage."sent to server"::No;
                SMSMessage."SMS Message" := ('You have guaranteed Mr./Mrs./M/s ' + "Client Name" + ' with Kes. ' + Format(LGuarantors."Amont Guaranteed") +
                ' for a loan application of Kes. ' + Format("Requested Amount") + '. If not, let us know. Thanks.');
                //Cust.SETRANGE(Cust."No.",LGuarantors."Member No");
                if Cust.Get(LGuarantors."Member No") then begin
                    SMSMessage."Telephone No" := Cust."Mobile Phone No";
                end;
                if Cust."Mobile Phone No" <> '' then
                    SMSMessage.Insert;
