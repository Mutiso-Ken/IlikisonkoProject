...
Codeunit 51516120 "PORTALIntegration"
{
    ...
    begin
        ...
        if objMember.Find('-') then begin
          ...
          objMember.CalcFields("Current Shares","Share Capital B Class", "Ordinary Savings");
        ...
        //objMember.CALCFIELDS("Share Cap", "Dividend Amount", "Un-allocated Funds");
          info:=Format(ROUND(objMember."Current Savings",0.01,'>'))+':'+Format(ROUND(objMember."Shares Retained",0.01,'>'))+':'+Format(ROUND(objMember."Current Shares",0.01,'>'))+':'+Format(ROUND(objMember."Ordinary Savings",0.01,'>'))+':'
        ...
        if DivProg.Find('-') then begin
          repeat
            if DivProg."Gross Dividends" < 1 then DivProg."Gross Dividends":=-1*DivProg."Gross Dividends";
             ...
             if DivProg."Member Deposits" < 1 then DivProg."Member Deposits":=-1*DivProg."Member Deposits";
             if DivProg."Qualifying Deposits" < 1 then DivProg."Qualifying Deposits":=-1*DivProg."Qualifying Deposits";
            dividend:=dividend+Format(DivProg.Date)+':::'+Format( DivProg."Gross Dividends")+':::'+Format(DivProg."Witholding Tax")+':::'+Format(DivProg."Net Dividends")+':::'+Format(DivProg."Member Deposits")+':::'
            +Format(DivProg."Qualifying Deposits")+'::::';
    ...
    var
        NextOfKin: Record "Member App Nominee";
    begin
    ...
    var
        ChangeReuqest: Record "Change Request";
    begin
    ...
    var
        NokChangeRequest: Record "Next Of Kin Change";
        NokLines: Record "Members Nominee Temp";
        NokKinDetails: Record "Members Nominee";
    begin
