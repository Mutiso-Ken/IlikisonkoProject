...
Codeunit 51516009 "SURESTEP Factory"
{
    ...
    procedure FnRunPostPenaltyOnLoanArrears()
    var
        ObjLoans: Record "Loans Register";
    ...
    begin
        ...
        repeat
        //VarNoofDaysinArrears:=ROUND((ObjLoans."Amount in Arrears"*-1)/ObjLoans."Loan Principle Repayment",1,'=');
          VarNoofDaysinArrears:=ObjLoans."No of Months in Arrears"*30;
        ...
        ObjLoans.Reset;
        ObjLoans.SetFilter(ObjLoans."Outstanding Balance",'>%1',0);
        ObjLoans.SetFilter(ObjLoans."Amount in Arrears",'<%1',0);
        ObjLoans.SetRange(ObjLoans."Loans Category-SASRA",ObjLoans."loans category-sasra"::Watch);
        if ObjLoans.FindSet then begin
          repeat
             VarPenaltyAmount:=0;
            ...
            BATCH_NAME:='PENALTY';
            DOCUMENT_NO:='PEN - '+Format(WorkDate);
            ...
            if ObjLoanType.Get(ObjLoans."Loan Product Type") then begin
              VarPenaltyAmount:=((ObjLoanType."Penalty Percentage"/100)*ObjLoans."Amount in Arrears")*-1;
              // MESSAGE('VarPenaltyAmount %1',VarPenaltyAmount);
              if VarPenaltyAmount<ObjGenSetUp."Loan Trasfer Fee-EFT" then begin
               VarPenaltyAmount:=ObjGenSetUp."Loan Trasfer Fee-EFT"
             end else
                VarPenaltyAmount:=VarPenaltyAmount;


               //=====================================================================================================2. CREDIT MEMBER REGISTRATION FEE A/C
                LineNo:=LineNo+10000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,GenJournalLine."transaction type"::"Ordinary Savings",
                GenJournalLine."account type"::Member,ObjLoans."Client Code",WorkDate, VarPenaltyAmount,'BOSA','',
                'Penalty On Arrears - '+ObjLoans."Loan  No.",ObjLoans."Loan  No.");

                //=======================================================================================================3. DEBIT TAX MEMBER DEPOSITS A/C
                LineNo:=LineNo+10000;
                FnCreateGnlJournalLine(BATCH_TEMPLATE,BATCH_NAME,DOCUMENT_NO,LineNo,GenJournalLine."transaction type"::"Penalty Charged",
                GenJournalLine."account type"::"G/L Account",ObjLoanType."Penalty Paid Account",WorkDate,VarPenaltyAmount*-1,'BOSA','',
                'Penalty On Arrears - '+ObjLoans."Loan  No.",ObjLoans."Loan  No.");
