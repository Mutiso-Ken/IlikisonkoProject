...
Codeunit 51516015 "Payroll Processing"
{
    ...
    var
        ...
        ContrAmt: Decimal;
        PayrollPeriodBuffer: Record "Payroll Period Buffer";
    ...
    var
        ...
        LoansRg: Record "Loans Register";
        LoansRegister: Record "Loans Register";
        intRate: Decimal;
        prEmployeeTransactionsPens: Record "Payroll Employee Transactions.";
        TestCode: Integer;
    begin
        //Initialize
        fnInitialize;
                ...
                // Prorate Allowances Here
                 ...
                 if prTransactionCodes.SubLedger<>prTransactionCodes.Subledger::" " then begin
                    if prTransactionCodes.SubLedger=prTransactionCodes.Subledger::Customer then begin
                        ...
                        Customer.Reset;
                        Customer.SetRange(Customer."Personal No",strEmpCode);
                        ...
                        RealCustomer.Reset;
                        RealCustomer.SetRange(RealCustomer."Staff No",strEmpCode);
                        if RealCustomer.Find('-') then begin
                           JournalAcc:=RealCustomer."No.";
                           JournalPostingType:=Journalpostingtype::Customer;
                        end;
        ...
        //Get Insurance Relief
         ...
         if EmployeeP.Find('-') then begin
              ...
              if PostingGroup.Get(EmployeeP."Posting Group") then
                begin
                  JournalAcc:=PostingGroup."Tax Relief";
                  end;
        
              //Insert Insurance Relief
              curTransAmount := CurrInsuranceRel;
              strTransDescription := 'Life Assurance Contribution';
          ...
          curTransAmount, 0, intMonth, intYear,'','',SelectedPeriod,Dept,JournalAcc,Journalpostas::" ",Journalpostingtype::" ",'',
          Coopparameters::none);
        ...
        begin
         ...
         fnUpdatePeriodTrans (strEmpCode, 'PSNRL', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
          curTransAmount, 0, intMonth, intYear,'','',SelectedPeriod,Dept,'',Journalpostas::" ",Journalpostingtype::" ",'',
          Coopparameters::none);
        end ;
        //ELSE
        // curReliefPersonal := 0;
        //Self contribution Calculation Amount
        
          ContrAmt:=0;
          prEmployeeTransactionsPens.Reset;
          prEmployeeTransactionsPens.SetRange(prEmployeeTransactionsPens."No.",strEmpCode);
          prEmployeeTransactionsPens.SetRange(prEmployeeTransactionsPens."Period Month",intMonth);
          prEmployeeTransactionsPens.SetRange(prEmployeeTransactionsPens."Period Year",intYear);
          prEmployeeTransactionsPens.SetRange(prEmployeeTransactionsPens."Transaction Code",'D09');
          prEmployeeTransactionsPens.SetRange(prEmployeeTransactionsPens.Suspended,false);
          if prEmployeeTransactionsPens.Find('-') then begin
            ContrAmt:=prEmployeeTransactionsPens.Amount;
          end;
        ...
        //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         //>Pension Contribution [self] relief
           ...
           if curPensionStaff+ContrAmt > 0 then begin
               if curPensionStaff +ContrAmt> curMaxPensionContrib then
               ...
               fnUpdatePeriodTrans (strEmpCode, 'PNSRLF'
               , TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
               curTransAmount, 0, intMonth, intYear,'','',SelectedPeriod,Dept,'',Journalpostas::" ",Journalpostingtype::" ",'',
               Coopparameters::none)
           end;
        ...
        //HOSP
         ...
         if curPensionStaff+ContrAmt > curMaxPensionContrib then
           curTaxablePay:= curGrossTaxable - (curSalaryArrears + curDefinedContrib +curMaxPensionContrib+curOOI+curHOSP+curNonTaxable+curInsuranceReliefAmount)
        ...
        if curPensionStaff+ContrAmt+curDefinedContrib > curMaxPensionContrib then
           curTransAmount := curTaxablePay+txBenefitAmt+curDefinedContrib
        else
         ...
         //curTransAmount := curTaxCharged+curReliefPersonal+(0.15*curInsuranceReliefAmount);
         ...
         curTransAmount := curPAYE-curReliefPersonal+(0.15*curInsuranceReliefAmount);
          ...
          if prEmployeeTransactions.Find('-') then begin
            ...
            repeat
              ...
              if prTransactionCodes.Find('-') then begin
               ...
               if prTransactionCodes."IsCo-Op/LnRep"=true then
                begin
        ...
        //CYRUS PRINCIPLE LOAN (Edited by Maureen)
                ...
                if (prTransactionCodes."Special Transaction"=prTransactionCodes."special transaction"::"Staff Loan") and
                   ...
                   /*IF prEmployeeTransactions."Loan Number"<>'' THEN BEGIN
                   IF LoansRg.GET(prEmployeeTransactions."Loan Number") THEN BEGIN
                     LoansRg.CALCFIELDS("Oustanding Interest");
                     curLoanInt:=LoansRg."Oustanding Interest";
                   END;
                 END;*/
                 if LoansRegister.Get(prEmployeeTransactions."Loan Number") then begin
                 intRate:=LoansRegister.Interest;
                 end;
                   curLoanInt:=fnCalcLoanInterest (strEmpCode, prEmployeeTransactions."Transaction Code",
                   intRate,prTransactionCodes."Repayment Method",
                   ...
                   //IF (curLoanInt<>0) THEN BEGIN
                          ...
                          fnUpdatePeriodTrans(strEmpCode, strTransCode, TGroup, TGroupOrder, TSubGroupOrder,
                            ...
                            prEmployeeTransactions."Sacco Membership No.", prEmployeeTransactions."Reference No",SelectedPeriod,Dept,
                ...
                case prTransactionCodes."Balance Type" of //[0=None, 1=Increasing, 2=Reducing]
                    prTransactionCodes."balance type"::None: begin
                         curTransBalance := 0;
                         //IF prTransactionCodes."Special Transaction"=prTransactionCodes."Special Transaction"::"Life Insurance" THEN
                         //curTransBalance:= prEmployeeTransactions.Balance+ curTransAmount;
                         end;
                    prTransactionCodes."balance type"::Increasing:
                        curTransBalance := prEmployeeTransactions.Balance+ curTransAmount;
                ...
                curTotalDeductions := fnGetTotalDeductions(strEmpCode,prEmployeeTransactions."Payroll Period");//curTotalDeductions + curTransAmount; //Sum-up all the deductions
                curTransAmount := curTransAmount;
                curTransBalance := curTransBalance;
                strTransDescription := prTransactionCodes."Transaction Name";
        ...
        //CYRUS PRINCIPLE LOAN
          ...
          until prEmployeeTransactions.Next=0;
             //GET TOTAL DEDUCTIONS
                  curTotalDeductions:= fnGetTotalDeductions(strEmpCode,SelectedPeriod);//curTotalDeductions+curNSSF + curNHIF + curPAYE + curPayeArrears;
                          curTransBalance:=0;
                          strTransCode := 'TOT-DED';
                          strTransDescription := 'TOTAL DEDUCTION';
                          TGroup := 'TOTALDEDUCTIONS'; TGroupOrder := 8; TSubGroupOrder := 9;
                          fnUpdatePeriodTrans(strEmpCode, strTransCode, TGroup, TGroupOrder, TSubGroupOrder,
          ...
          curNetPay := curGrossPay - (curTotalDeductions+IsCashBenefit+Teltax2);
          ...
          if  curTransAmount< 0 then
          Message('The net pay for the %1 is less than 0',strEmpCode);
    ...
    begin
        ...
        with prPeriodTransactions do begin
            ...
            if GroupOrder=8  then begin
              if ("Transaction Code"<>'PNSR') and ("Transaction Code"<>'D05') and ("Transaction Code"<>'TOT-DED')then
              "Sub Group Order" := 5 else
              "Sub Group Order" := SubGroupOrder;
            end;
    ...
    begin
        ...
        PAYE := PAYE + ROUND((curTempAmount * (prPAYE.Rate / 100)),0.05,'>');
    ...
    begin
        ...
        //prEmployeeTransactions.SETRANGE(prEmployeeTransactions."No.",'KPSS091');
           ...
           //Insert record for the next period
            with prEmployeeTrans do begin
              ...
              "Original Amount":= prEmployeeTransactions."Original Amount";
               ...
               Membership:=prEmployeeTransactions."Sacco Membership No.";
    ...
    begin
        ...
        prEmployee.Reset;
        prEmployee.SetRange(prEmployee.Status,prEmployee.Status::Inactive);
    ...
    begin
        ...
        curLoanInt := 0;
        //Back Date
        PayrollPeriodBuffer.FindFirst;//For BackDate
        ...
        //FirstDayofMonth:=CALCDATE('<-CM>',TODAY);
         //RefDate:=CALCDATE('19D',FirstDayofMonth);
        ////CALCULATE Interest Based on Filter
        ...
        //FirstDayofMonth:=CALCDATE('<-CM>',TODAY);
        //LastDayMonth:=CALCDATE('<CM>',TODAY);
        
        
        
        /*LoanRepaymentSchedule.RESET;
        LoanRepaymentSchedule.SETRANGE("Loan No.",LoanNo_);
        LoanRepaymentSchedule.SETFILTER("Repayment Date",'%1..%2',FirstDayofMonth,LastDayMonth);
        IF LoanRepaymentSchedule.FIND('-') THEN BEGIN
        
         IF loans.GET(LoanNo_) THEN BEGIN
         loans.CALCFIELDS(loans."Outstanding Balance",loans."Oustanding Interest");
         LnInterest:=LoanRepaymentSchedule."Monthly Interest";
         END;
    ...
    procedure fnGetTotalDeductions(EmpCode: Code[20];PPeriod: Date) totDed: Decimal
    var
        prPeriodTrans: Record "prPeriod Transactions.";
    begin
        prPeriodTrans.Reset;
        prPeriodTrans.SetRange(prPeriodTrans."Employee Code",EmpCode);
        prPeriodTrans.SetRange(prPeriodTrans."Payroll Period",PPeriod);
        prPeriodTrans.SetFilter(prPeriodTrans."Group Text",'%1|%2','DEDUCTIONS','STATUTORIES');
        if prPeriodTrans.Find('-') then begin
        prPeriodTrans.CalcSums(Amount);
        totDed:=prPeriodTrans.Amount;
        end;
    end;
}
