...
Codeunit 51516026 "CloudPESALivetest"
{

    trigger OnRun()
    begin
        //MESSAGE(RegisteredMemberDetailsUSSD('254727758318','000023456'));
        //MESSAGE(OutstandingLoansUSSD('0702673946'));
        //MESSAGE(LoanRepayment('000008','BLN00145','QWER122344',5000));
        //MESSAGE(MiniStatement('254727758318','124000014'));
        //MESSAGE(AccountBalanceUSSD('+254702636777','25195455'));
        //MESSAGE(Accounts('0702636777','99556'));
        //MESSAGE(MemberAccounts('0702636777'));
        //MESSAGE(AccountBalance('0702636777','15909652'));
        //MESSAGE(SurePESARegistration());Kanisa
        //MESSAGE(WSSAccount('0702636777'));
        //SMSMessage('3457111','1234','0727758318','sms service test by mutinda');
        //MESSAGE(OutstandingLoanName('0702636777'));
        //MESSAGE(UpdateSurePESARegistration('BES000004'));
        //MESSAGE(LoanGuarantorsUSSD('LB15205','0729534407','27897'));
        //MESSAGE(MiniStatement('0729534407', '8855255'));
        //MESSAGE(OutstandingLoans('0729534407'));
    ...
    var
        ...
        ExcDuty: Decimal;
        TempBalance: Decimal;
        BOSATransSchedule: Record "BOSA Transfers";
    ...
    begin
        begin
            ...
            if Members.Find('-') then begin
                ...
                accounts := accounts + '::::' + Members."No." + ':::' + Format(MemberLedgerEntry."transaction type"::"Ordinary Savings");
                //END;
    ...
    begin
        ...
        if SurePESATrans.Find('-') then begin
            ...
            if Members.Find('-') then begin
                MemberLedgerEntry.Reset;
                MemberLedgerEntry.SetRange(MemberLedgerEntry."Customer No.", Members."No.");
                MemberLedgerEntry.SetRange(MemberLedgerEntry."Transaction Type", MemberLedgerEntry."transaction type"::"Ordinary Savings");
                if MemberLedgerEntry.Find('-') then begin
                    repeat
                ...
                //samount:=KanisaFactory.KnGetDemandSavingsBalance(Members."No.");
                ...
                if (TempBalance > amount) then begin
                    ...
                    GenJournalLine."Transaction Type" := GenJournalLine."transaction type"::"CRB Fees";
                    ...
                    if accTo = 'Kanisa' then begin
                        ...
                        GenJournalLine."External Document No." := Members."No.";
                        GenJournalLine."Posting Date" := Today;
                        GenJournalLine."Transaction Type" := GenJournalLine."transaction type"::"Ordinary Savings";
                        GenJournalLine.Description := Format(GenJournalLine."transaction type"::"Ordinary Savings");
                        GenJournalLine.Amount := -amount;
                        GenJournalLine.Validate(GenJournalLine.Amount);
                        if GenJournalLine.Amount <> 0 then
                            GenJournalLine.Insert;
                    ...
                    result := 'TRUE';
                    msg := 'You have transfered KES ' + Format(amount) + ' from Account Jiokoe Savings to ' + accTo + ' thank you for using Kanisa Sacco Mobile.';
                    ...
                    ' .Thank you for using Kanisa Sacco Mobile.';
        ...
        begin
            ...
            if Members.Find('-') then begin
                ...
                if MemberLedgerEntry.Find('-') then begin
                    ...
                    accounts := Members."No." + ':::' + Format(MemberLedgerEntry."transaction type"::"Ordinary Savings");
                end;
                //END;
            end
            else begin
            ...
            if (TempBalance > amount) then begin
                ...
                if LoansRegister.Find('+') then begin
                    ...
                    if (LoansRegister."Outstanding Balance" > 0) then begin
                        ...
                        SurePESATrans.Insert;
                        result := 'TRUE';
                        msg := 'You have transfered KES ' + Format(amount) + ' from Account jiokoe savings to ' + LoansRegister."Loan Product Type Name" + ' thank you for using Kanisa Sacco Mobile.';
            ...
            if Members.Find('-') then begin
                ...
                if LoansTable.Find('-') then begin
                    repeat
                        ...
                        // amount:=LoansTable.Installments-KanisaFactory.kt
    ...
    begin
        begin
            ...
            Members.SetRange(Members."Phone No.", Phone);
            if Members.Find('-') then begin
                reginfo := 'Member No: ' + Members."No." + ',  Name: ' + Members.Name + ',  ID No: ' + Format(Members."ID No.") + ',  Payroll No: ' + Members."Payroll/Staff No2" + ',  Email :' + Members."E-Mail" + ',Employer:' + Members."Employer Name";
    ...
    begin
        begin
            ...
            SMSMessage(DocNumber, Members."No.", Phone, ' Your Account balance is Kshs: ' + Bal + '. Thank you for using Kanisa Mobile');
        end;
    end;
    ...
    begin
        begin
            ...
            if JuniorAcc(phone) <> 'NULL' then begin
                junior := 'Mwanangu Savings= KES ' + JuniorAcc(phone);
                accounts := accounts + junior + ' , ';
            end;

            if BenevolentFund(phone) <> 'NULL' then begin
                benevolent := 'Jiokoe Savings= KES ' + BenevolentFund(phone);
                accounts := accounts + benevolent;
            end;
    ...
    begin
        begin
            ...
            Members.SetRange(Members."Phone No.", phone);
            if Members.Find('-') then begin
                //shares:=FORMAT( KanisaFactory.KnGetDemandSavingsBalance(Members."No."));
            end else begin
                shares := '0';
            end;
        end;
    ...
    begin
        ...
        if Members.Find('-') then begin
            MemberLedgerEntry.Reset;
            MemberLedgerEntry.SetRange(MemberLedgerEntry."Customer No.", Members."No.");
            MemberLedgerEntry.SetRange(MemberLedgerEntry."Transaction Type", MemberLedgerEntry."transaction type"::"Loan Insurance Charged");
            if MemberLedgerEntry.Find('-') then begin
                repeat
    ...
    begin
        begin
            ...
            if GenJournalLine.Find('-') then begin
                ...
                msg := 'Dear ' + Vendor.Name + ' your acc: ' + Vendor."No." + ' has been credited with Ksh. ' + Format(Amount) + ' Thank you for using Kanisa M-Banking';
                ...
                if LoansRegister."Outstanding Balance" > 0 then begin
                    ...
                    if GenJournalLine.Find('-') then begin
                        repeat
                            GLPosting.Run(GenJournalLine);
                        until GenJournalLine.Next = 0;
                        msg := 'Dear ' + Members.Name + ' your  ' + LoansRegister."Loan Product Type Name" + ' has been credited with Ksh ' + Format(PaybillTrans.Amount) + ' Thank you for using Kanisa M-Banking';
    ...
    begin
        begin
            ...
            if GenJournalLine.Find('-') then begin
                ...
                PaybillTrans.Modify;
                res := 'TRUE';
                msg := 'Dear ' + Members.Name + ' your acc: ' + Members."No." + ' has been credited with Ksh' + Format(amount) + ' Thank you for using Kanisa SACCO Mobile';
    ...
    begin
        ...
        if Members.Find('-') then begin
            ...
            if (TempBalance > amount) then begin
                ...
                SurePESATrans.Insert;
                result := 'TRUE';
                msg := 'You have purchased airtime worth KES ' + Format(amount) + ' from Account ' + Vendor.Name + ' thank you for using Kanisa Sacco Mobile.';
    ...
    begin
        ...
        IF ObjMembReg.GET(MembNo) THEN
          BEGIN
            InsAmount:=KanisaFactory.KnGetInsurancePaid(MembNo);
            IF InsAmount > 0 THEN
              BEGIN
